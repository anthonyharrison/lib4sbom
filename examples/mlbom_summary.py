...
from lib4sbom.data.modelcard import SBOMModelCard, ModelDataset, ModelGraphicset
                    ...
                    if package.get("modelCard") is not None:
                        ...
                        if "architecture" in modelCard:
                            if not title:
                                print ("Model Parameters")
                                title = True
                            print (f"\tArchitecture Familiy: {modelCard['architecture']}")
                        ...
                        if "dataset" in modelCard:
                            ...
                            for dataset in modelCard['dataset']:
                                ...
                                if "sensitive_data" in dataset:
                                    print (f"\tSensitive Data: {dataset['sensitive_data']}")
                                # Graphics
                                if "graphics" in dataset:
                                    print("Graphics")
                                    print(f"\tDescription: {dataset['graphics']['description']}")
                                ...
                                if "custodian" in dataset:
                                    ...
                                    for custodian in dataset["custodian"]:
                                        print(f"\tOrganization {custodian.get('organization')}\tContact {custodian.get('contact')}")
                                if "steward" in dataset:
                                    print ("\tStewards")
                                    for steward in dataset["steward"]:
                                        print(f"\tOrganization {steward.get('organization')}\tContact {steward.get('contact')}")
                                if "owner" in dataset:
                                    print ("\tOwners")
                                    for owner in dataset["owner"]:
                                        print(f"\tOrganization {owner.get('organization')}\tContact {owner.get('contact')}")
                        ...
                        if "performance" in modelCard:
                            ...
                            for performance in modelCard["performance"]:
                                print (f"\tType: {performance[0]} Value {performance[1]} Slice: {performance[2]} Lower Bound: {performance[3]} Upper Bound : {performance[4]}")
                        if "graphics" in modelCard:
                            if not title:
                                print ("Quantitative Analysis")
                                title = True
                            print ("Graphics")
                            print (f"\tDescription: {modelCard['graphics']['description']}")
                        ...
                        if "ethicalrisk" in modelCard:
                            ...
                            for consideration in modelCard["ethicalrisk"]:
                                print (f"\tName: {consideration[0]} Mitigation Strategy: {consideration[1]}")
                        if "fairness" in modelCard:
                            ...
                            for assessment in modelCard["fairness"]:
                                print (f"\tGroup at risk: {assessment[0]} Benefirs: {assessment[1]} Harms: {assessment[2]} Mitigation Strategy: {assessment[3]}")
