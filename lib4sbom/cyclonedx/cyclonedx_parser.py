...
class CycloneDXParser:
        ...
        for license in license_element:
            if "license" in license:
                ...
                if "name" in license["license"]:
                    ...
                    if id is None:
                        # No explicit id; check if the name is really a known SPDX id
                        resolved = self.license_scanner.find_license_id(name)
                        if resolved not in [self.license_scanner.DEFAULT_LICENSE, ""]:
                            id = resolved
    ...
    def parse_cyclonedx_json(self, data: dict):
        ...
        files = {}
        relationships = []
        # DESCRIBES is generated by the generator for the document-to-root relationship
