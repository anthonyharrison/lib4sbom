...
class SPDXParser:
    ...
    def parse_spdx_rdf(self, sbom_file):
        """parses SPDX RDF SBOM file extracting some SBOM data"""
        with open(sbom_file) as f:
            lines = f.readlines()
        spdx_document = {}
        files = {}
                ...
                if line.strip().startswith("<spdx:name>"):
                    ...
                    if license_match is None:
                        ...
                        if "#" in license:
                            ...
                            license = license[
                                :-1
                            ].upper()  # Remove trialing " and capitalise
                    # To handle case where license appears before version
        ...
        return spdx_document, files, packages, relationships
    ...
    def parse_spdx_xml(self, sbom_file):
        """parses SPDX XML SBOM file extracting some SBOM data"""
